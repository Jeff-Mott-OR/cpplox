...
namespace
{
    ...
    struct Compiler
    {
        ...
        void emit_setter(const Chunk::Token& identifier_token)
        {
        ...
        void track_local(const Chunk::Token& identifier_token, bool initialized = true)
        {
        ...
        std::vector<Chunk::Tracked_upvalue>::const_iterator track_upvalue(std::string_view identifier_name)
        {
        ...
        void compile_primary_expression()
        {
            switch (token_iter->type) {
                ...
                case Token_type::false_: {
                    function_chunks.back()->chunk.emit<Opcode::false_>(make_owning_token(*token_iter++));
                    break;
                }
                ...
                case Token_type::nil: {
                    function_chunks.back()->chunk.emit<Opcode::nil>(make_owning_token(*token_iter++));
                    break;
                }
                ...
                case Token_type::super: {
                    const auto super_token = make_owning_token(*token_iter++);

                    emit_getter(make_owning_token(Token{Token_type::this_, "this", super_token.line}));
                    emit_getter(super_token);

                    ensure_token_is(*token_iter++, Token_type::dot);
                    ensure_token_is(*token_iter, Token_type::identifier);
                    const auto method_name_token = make_owning_token(*token_iter++);
                ...
                case Token_type::true_: {
                    function_chunks.back()->chunk.emit<Opcode::true_>(make_owning_token(*token_iter++));
                    break;
                }
            }
        }
        ...
        {
            auto maybe_callee_token = make_owning_token(*token_iter);
            ...
            while (token_iter->type == Token_type::left_paren || token_iter->type == Token_type::dot) {
                ...
                if (advance_if_match(Token_type::dot)) {
                    ensure_token_is(*token_iter, Token_type::identifier);
                    const auto property_name_token = make_owning_token(*token_iter++);
        ...
        void compile_unary_precedence_expression()
        {
            if (token_iter->type == Token_type::minus || token_iter->type == Token_type::bang) {
                const auto unary_op_token = make_owning_token(*token_iter++);
        ...
        void compile_multiplication_precedence_expression()
        {
            ...
            while (token_iter->type == Token_type::star || token_iter->type == Token_type::slash) {
                const auto binary_op_token = make_owning_token(*token_iter++);
        ...
        void compile_equality_precedence_expression()
        {
            ...
            while (token_iter->type == Token_type::equal_equal || token_iter->type == Token_type::bang_equal) {
                const auto equality_token = make_owning_token(*token_iter++);
        ...
        void compile_and_precedence_expression()
        {
            ...
            while (token_iter->type == Token_type::and_) {
                const auto and_token = make_owning_token(*token_iter++);
        ...
        void compile_or_precedence_expression()
        {
            ...
            while (token_iter->type == Token_type::or_) {
                const auto or_token = make_owning_token(*token_iter++);
            ...
            if (token_iter->type == Token_type::identifier || token_iter->type == Token_type::this_) {
                ...
                if (peek_ahead_iter->type == Token_type::equal) {
                    ...
                    if (property_name_tokens.empty()) {
                        ...
                        const auto last_property_name_token = make_owning_token(property_name_tokens.back());
        ...
        void compile_expression_statement()
        {
            ...
            function_chunks.back()->chunk.emit<Opcode::pop>(make_owning_token(*token_iter++));
        }
        ...
        unsigned int compile_function_rest(const Chunk::Token& source_map_token)
        {
            ...
            if (! advance_if_match(Token_type::right_paren)) {
                do {
                    ensure_token_is(*token_iter, Token_type::identifier);
                    track_local(make_owning_token(*token_iter++));
                    ++param_count;
                } while (advance_if_match(Token_type::comma));
                ensure_token_is(*token_iter++, Token_type::right_paren);
            }
        ...
        void compile_statement()
        {
            switch (token_iter->type) {
                ...
                case Token_type::if_: {
                    const auto if_token = make_owning_token(*token_iter++);
                ...
                case Token_type::for_: {
                    const auto for_token = make_owning_token(*token_iter++);
                ...
                case Token_type::left_brace: {
                    ...
                    ensure_token_is(*token_iter, Token_type::right_brace);
                    pop_top_scope_depth(make_owning_token(*token_iter++));

                    break;
                }

                case Token_type::print: {
                    const auto print_token = make_owning_token(*token_iter++);
                ...
                case Token_type::while_: {
                    const auto while_token = make_owning_token(*token_iter++);
        ...
        void compile_declaration()
        {
            switch (token_iter->type) {
                ...
                case Token_type::class_: {
                    const auto class_token = make_owning_token(*token_iter++);

                    ensure_token_is(*token_iter, Token_type::identifier);
                    const auto class_name_token = make_owning_token(*token_iter++);
                    ...
                    if (advance_if_match(Token_type::less)) {
                        ensure_token_is(*token_iter, Token_type::identifier);
                        const auto superclass_name_token = make_owning_token(*token_iter++);
                        ...
                        ++scope_depth;
                        track_local(make_owning_token(Token{Token_type::super, "super", superclass_name_token.line}));
                        ...
                        maybe_pop_superclass_scope = [this, superclass_name_token] {
                            ...
                            pop_top_scope_depth(make_owning_token(Token{Token_type::super, "super", superclass_name_token.line}));
                ...
                case Token_type::var: {
                    const auto var_token = make_owning_token(*token_iter++);

                    ensure_token_is(*token_iter, Token_type::identifier);
                    const auto variable_name_token = make_owning_token(*token_iter++);
                    if (! is_global_scope()) {
