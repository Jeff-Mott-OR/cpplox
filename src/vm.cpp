...
namespace motts::lox
{
    ...
    void VM::run(GC_ptr<Function> function)
    {
        assert(! ! function && "Expect non-null.");

        if (debug_) {
            os_ << "\n# Running chunk:\n\n" << function->chunk << '\n';
        }

        run(gc_heap_.make<Closure>(function), 0);
    }
        ...
        while (bytecode_iter != bytecode_end) {
            ...
            switch (opcode) {
                default: {
                    ...
                    os << "[Line " << source_map_tokens[bytecode_index].line << "] Error: Unexpected opcode " << opcode
                       << ", generated from source \"" << *source_map_tokens[bytecode_index].lexeme << "\".";
    ...
    void VM::opcode_add(std::span<const Source_map_token> source_map_tokens, std::ptrdiff_t bytecode_index)
    {
    ...
    )
    {
        ...
        if (const auto maybe_bound_method = std::get_if<GC_ptr<Bound_method>>(&maybe_callable)) {
        ...
        if (const auto maybe_native_fn = std::get_if<GC_ptr<Native_fn>>(&maybe_callable)) {
            const auto& native_fn = *maybe_native_fn;
            const auto return_value = native_fn->fn({stack_.end() - arg_count, stack_.end()});
            *(stack_.end() - arg_count - 1) = return_value;
            stack_.erase(stack_.end() - arg_count, stack_.end());
    ...
    )
    {
        ...
        // See hot loop micro-optimization: Reinterpret variants
        const auto& function = reinterpret_cast<const GC_ptr<Function>&>(constants[fn_constant_index]);
    ...
    void VM::opcode_divide(std::span<const Source_map_token> source_map_tokens, std::ptrdiff_t bytecode_index)
    {
    ...
    void VM::opcode_get_local(std::size_t stack_begin_index, std::vector<std::uint8_t>::const_iterator& bytecode_iter)
    {
    ...
    )
    {
        ...
        // See hot loop micro-optimization: Assign directly to stack
        *(stack_.end() - 2) = new_bound_method;
        stack_.pop_back();
    }

    void VM::opcode_get_upvalue(
        std::vector<std::uint8_t>::const_iterator& bytecode_iter,
        std::span<GC_ptr<Upvalue>> upvalues
    )
    {
    ...
    void VM::opcode_greater(std::span<const Source_map_token> source_map_tokens, std::ptrdiff_t bytecode_index)
    {
        const auto maybe_double_rhs = std::get_if<double>(&*(stack_.cend() - 1));
        const auto maybe_double_lhs = std::get_if<double>(&*(stack_.cend() - 2));
    ...
    void VM::opcode_inherit(std::span<const Source_map_token> source_map_tokens, std::ptrdiff_t bytecode_index)
    {
    ...
    void VM::opcode_less(std::span<const Source_map_token> source_map_tokens, std::ptrdiff_t bytecode_index)
    {
        const auto maybe_double_rhs = std::get_if<double>(&*(stack_.cend() - 1));
        const auto maybe_double_lhs = std::get_if<double>(&*(stack_.cend() - 2));
    ...
    void VM::opcode_multiply(std::span<const Source_map_token> source_map_tokens, std::ptrdiff_t bytecode_index)
    {
        const auto maybe_double_rhs = std::get_if<double>(&*(stack_.cend() - 1));
        const auto maybe_double_lhs = std::get_if<double>(&*(stack_.cend() - 2));
    ...
    void VM::opcode_negate(std::span<const Source_map_token> source_map_tokens, std::ptrdiff_t bytecode_index)
    {
    ...
    void VM::opcode_return_(std::size_t stack_begin_index, std::span<GC_ptr<Upvalue>> open_upvalues)
    {
    ...
    void VM::opcode_set_local(std::size_t stack_begin_index, std::vector<std::uint8_t>::const_iterator& bytecode_iter)
    {
        const auto local_stack_index = *bytecode_iter++;
    ...
    void VM::opcode_subtract(std::span<const Source_map_token> source_map_tokens, std::ptrdiff_t bytecode_index)
    {
        const auto maybe_double_rhs = std::get_if<double>(&*(stack_.cend() - 1));
        const auto maybe_double_lhs = std::get_if<double>(&*(stack_.cend() - 2));
