...
namespace motts { namespace lox {
    struct Expr {
        ...
        linter visitor could return a list of strings. In Nystrom's Java code, he achieved this by making the method --
        just the method -- generic, where the return type was a type variable.
        ...
        After some reading and asking some questions, I discovered that Java's type variables de-sugar to `Object`. That
        is, given the Java method declaration `<R> R accept(Visitor<R> visitor);`, rather than generating a new method
        for each type as C++ templates do, Java instead implements just a single method as if it were `Object
