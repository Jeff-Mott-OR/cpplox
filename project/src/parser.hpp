...
namespace motts { namespace lox {
    /*
    Nystrom's Java code uses a `Parser` class with a single public method `parse`. That may be because Java has no free functions;
    everything must be in a class. In earlier commits, I mirrored that implementation, but in C++, free functions are not only possible but
    preferred, so I replaced the `Parser` class with a single `parse` free function.
