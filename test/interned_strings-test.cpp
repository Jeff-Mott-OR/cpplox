...
BOOST_AUTO_TEST_CASE(interned_strings_are_weakref_and_delete_if_collected)
{
    ...
    const auto gc_ptr_not_marked = interned_strings.get("hello");
    const auto gc_ptr_not_marked_dup = interned_strings.get("hello");
    ...
    interned_strings.get("dummy");

    const auto gc_ptr_after_collect = interned_strings.get("hello");
